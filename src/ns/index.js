...
function provisionNameServers () {
  ...
  const nameServersInput = document.getElementById('nameservers')

  if (nameServersInput) {
    // Split the input by newlines and remove any leading/trailing whitespace
    const nameServersArray = nameServersInput.value.split('\n').map(function (server) {
      return server.trim()
    })
      ...
      .then(async function (response) {
        if (response.ok) {
          ...
          if (response.status === 400) {
            errorText = "Malformed input. Please check that you've entered AWS Route 53 nameservers, one per line."
